...
export const Envelopes = YumiSignResource.extend({
    ...
    retrieve(id, params) {
        return this._makeRequest(`/${id}${(params === null || params === void 0 ? void 0 : params.session) ? '?session=1' : ''}`, { method: 'GET' }).then((envelope) => transformEnvelope(envelope));
    },
    list(ids) {
        return this._makeRequest(`?ids[]=${ids.join('&ids[]=')}`, { method: 'GET' }).then((bulkItems) => bulkItems.map((bulkItem) => {
            ...
            return Object.assign({ response: response ? transformEnvelope(response) : response }, rest);
        }));
    },
    create(params) {
