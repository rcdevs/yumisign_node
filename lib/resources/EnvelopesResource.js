...
module.exports = YumiSignResource.extend({
    ...
    retrieve(id, params) {
        return this._makeRequest(`/${id}${(params === null || params === void 0 ? void 0 : params.session) ? '?session=1' : ''}`, { method: 'GET' }).then((envelope) => (0, transformer_1.transformEnvelope)(envelope));
    },
    list(ids) {
        return this._makeRequest(`?ids[]=${ids.join('&ids[]=')}`, { method: 'GET' }).then((bulkItems) => bulkItems.map((bulkItem) => {
            ...
            return Object.assign({ response: response ? (0, transformer_1.transformEnvelope)(response) : response }, rest);
        }));
    },
    create(params) {
        ...
        return this._makeRequest('', {
            method: 'POST',
            body,
        }).then((envelope) => (0, transformer_1.transformEnvelope)(envelope));
    },
