...
module.exports = YumiSignResource.extend({
    ...
    listActions(params) {
        ...
        return this._makeRequest(endpoint, { method: 'GET' }).then((paginatedList) => {
            ...
            return Object.assign({ items: items.map((item) => (0, transformer_1.transformAction)(item)) }, rest);
        });
    },
    listSignedEnvelopes(params) {
        ...
        return this._makeRequest(endpoint, { method: 'GET' }).then((paginatedList) => {
            const { items } = paginatedList, rest = __rest(paginatedList, ["items"]);
            return Object.assign({ items: items.map((item) => (0, transformer_1.transformEnvelope)(item)) }, rest);
        });
    },
});
